...
% The preamble ends with the command \begin{document}
	\begin{document}
		...
		I've also implemented an alternative method that use instead of the simplified model describe earlier the original model but with the speed fixed to the exact velocity of the observation zone. This method should include better optimum than the other models but test show slightly worse result (observation time around 30 minutes shorter). It is very likely that these error come from the fact that the optimum of this model are less obvious making them more difficult to find. Additionally this model run also slightly slower because of the differential we have to solve.
		...
		\section{low thrust transfert}
			...
			with this example a transfert between two ascending or descending observations typically consume 300 grams of fuel. At this rate we could achieve mission of multiple years (around 6 years) if we consider the fuel as the only limiting factor in mission time.
			 ...
			 \\ \\
			 The second code is made in Julia using a library of Optimal Control. Its goal is to test transfert from one observation to another one by solving a optimal control problem.
			 ...
			 There also some scenario that were barrely explored that could be usefull, for example the idea of linking an descengin observation and a ascending one taking advantage of their high similirality because of the Sun movement . Then slowy moving to another observation 6 mounth later could be a good scenarion if the original required too much energy. There also the third observation that was discover later and havn't be studied at all and could be a good option for solar sail mission even if they seem less efficient than the ones already founded.
	
			 To conclude there are also the unknown caused that could explain the existence of the three optimum. The conjecture made so far was that making an observation by going in the same direction of the observation zone when it is at proximity of the same energy circular orbit has show to be the solution without being the exact one (using this method in the simplified model don't give good result in the true model). I think there are a mathematical reason of the existence of theses peak that, if we simplify some value and neglected other, become this conjecture.
