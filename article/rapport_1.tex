...
% The preamble ends with the command \begin{document}
	\begin{document}
		...
		we can achieve zero velocity observation if the velocity of the device at the observation zone ($\sqrt{\frac{2\mu}{R_s}-\frac{\mu}{a_l}$) is the same as the velocity of the observation zone. As the position of the point $P_{2x}$ is given by the following formula : 
			...
			the acceleration of the thruster can be take in account by adding a vector the the equations of speed : \\
			...
			=\begin{bmatrix}
				v\\
				a+\sum\limits _{\beta \in B }\frac{\mu _{\beta }}{\| r_{\beta } -r\| ^{3}}( r_{\beta } -r)
			\end{bmatrix}
			$$
			...
			$$
			H(x,p,a)=\begin{bmatrix}
				v\\
				a+\sum\limits _{\beta \in B }\frac{\mu _{\beta }}{\| r_{\beta } -r\| ^{3}}( r_{\beta } -r)
			\end{bmatrix}^t\cdot p-\|a\|
			$$
			...
			we know that $\forall t$ , $a=\arg\max_{a\in B(0,A_{max})}H(x,p,a)$. We can rewrite the hamiltonian as follow : 
			$$
			\begin{align}
				H(x,p,a)=&C(x,p)+a\cdot p-\|a\| \\
				=&C(x,p)+\|a\|(\|p_v\|\cos(\alpha_{\widehat{ap_v}})-1)
			\end{align}
			$$ 
			...
			$$
			\begin{cases}
				\| a\| =A_{max} & \text{if } \|p_v\|\cos(\alpha_{\widehat{ap_v}})\ge1\\
				\| a\| =0 & \text{else}
			\end{cases}
			$$
			
			we can then observe that $\|p_v\|\cos(\alpha_{\widehat{ap_v}})$ cannot be greater than $1$ if $\|p_v\| < 1$. Also if $\|p_v\| \ge 1$, it is obvious that taking $\alpha_{\widehat{ap_v}}=0$ (i.e. $a$ and $p_v$ are collinear) maximize the factor $\|p_v\|\cos(\alpha_{\widehat{ap_v}})$. So we can conclude the following formula :
			
			$$
			\begin{cases}
				\| a\| =A_{max}\widehat{p_v} & \text{if }\|p_v\| \ge 1\\
				\| a\| =0 & \text{else}
			\end{cases}
			$$
