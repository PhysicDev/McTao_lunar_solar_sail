...
% The preamble ends with the command \begin{document}
	\begin{document}
		...
		$$
		\begin{equation}
			P_3=R_l+\hat{P_{3x}}D^{-1}R_{ls} 
		\end{equation}
		$$
		$$
		\begin{equation}
			P_1=R_l+\hat{P_{1x}}D^{-1}R_{ls} 
		\end{equation}
		$$
		...
		working with the simplified 2D problem with the moon orbit being simplified as a perfect circle, the Sun being fixed and the observation zone distance from the Moon being fixed at the Earth Moon distance. we can get an observation that can give very long observation time :
		
		as most of the possible solution give observation time of a few minutes, there are two point where we can obtain observations time up to 
		hours. these point happen when the observation zone cross the lunar orbit. these observation time are very high because of the fixed period of the device, this mean that the speed of the device match the speed of the Moon when they are at the same altitude, meaning that we can cross the observation zone with a relative speed of zero.
		
		Observations of the trajectory show that the satellite describe a loop inside the observation zone, meaning that it is possible to get two fairly long observation very near to each other if the tip of the loop is outside of the observation zone. ascending if there is a good chance that these solutions are less efficient that a solution with the entire loop inside the observation zone, it could be usefull to improve the objective function to detect when the object reenter the observation zone after a short time.
		...
		theses observation happen when the moon is between its first crescent and first quarter, the same way, the observation can happen when the Moon is between its last quarter and last crescent.
		%When we are working with a simple 2D problem with circular Moon orbit and the Sun fixed, there are an optimum near the value $(2\pi/3,0.5,38/45\pi,0)$ that give observation time of almost 20h.
