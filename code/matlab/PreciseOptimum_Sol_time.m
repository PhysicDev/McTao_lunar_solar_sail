...
payoff2=zeros(1,(N+1));
time=(0:N)*(param.maxT-1)/N;
disp("computing trajectory")
for i=1:(N+1)
    payoff(i)=fun(time(i));
    payoff2(i)=BestVal123(time(i));
    i
end
hold off
fig=figure()
...
% altitude.
%==========================================================================
function [out,Pm,Pz,V]=BestVal123(t)
    global param;
    %pos du soleil
...
% altitude.
%==========================================================================
function [out,Pm,Pz,V]=BestVal2OK(t)
    global param;
    %pos du soleil
    ...
    %step 4: calculer la vitesse du satelite:
    VsN=abs(sqrt(param.Muter/norm(P2+posEarth)));

    %retourner le payoff (très facile)
    %out=(norm(posLun)-norm(P2))^2;
    out=(norm(VP2-Ve)-VsN)^2;
    %Test=param.Muter*(2/norm(P2)-1/param.as);
    Pm=posLun;
    Pz=P2;
    V=VP2*VsN/norm(VP2);%getSpeed(param.al,param.el,param.wl,param.Wl,param.Il,f);
end
...
function [out,Pm,Pz,V]=BestVal(t)
    ...
    tspan = [t, t+1];
    [time, Y,TE,YE,IE] = ode45(@Dfl, tspan, x0, optionsODE);
    T=TE;
    tspan = [t, t-1];
    [time, Y,TE,YE,IE] = ode45(@Dfl, tspan, x0, optionsODE);
    T=T-TE;
    ...
    Pm=[0,0,0];
    Pz=x0(1:3);
    V=x0(4:6);%VP2*VsN/norm(VP2);%getSpeed(param.al,param.el,param.wl,param.Wl,param.Il,f);
end
...
function x0=makeX0DE1(X)
    ...
    XP3=Xl'+(param.DP3/param.Dter)*(Xl'-Xst');
    XP1=Xl'+(param.DP1/param.Dter)*(Xl'-Xst');

    %radius:
    R=Xl'+param.DP2/param.Dter*(Xl'-Xst');
    Rn=norm(R);



    V=Vl'+param.DP2/param.Dter*(Vl'-Vst');
